    ...
    <div className='grid grid-cols-2 gap-4'>
      {data.data.data.map(pedido =>(
        <div key={pedido.id} className='p-5 bg-white shadow space-y-2 border-b'>
          ...
          {pedido.productos.map(producto => (
            <div key={producto.id}
                 className='border-b border-b-slate-200 last-of-type:border-none py-4'>
                   <p>Fecha: {formatearFecha(pedido.updated_at)}</p>
                   {/* <p>Fecha: {pedido.created_at.toLocaleDateString()}</p> */}
          ...
          <button type="button" 
                 className='bg-indigo-600 hover:bg-indigo-800 cursor-pointer px-5 py-2 rounded
                 uppercase font-bold text-white text-center w-full'
